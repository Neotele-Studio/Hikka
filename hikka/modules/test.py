...
# 👾 код был взят из фрока @codrago
# edited @ruisask & @matubuntu
...
class TestMod(loader.Module):
        ...
        self.config = loader.ModuleConfig(
            loader.ConfigValue(
                ...
                (
                    ...
                    " Hikka will try to determine, which client caused . E.g. there"
            ...
            loader.ConfigValue(
                "ping_text",
                "<emoji document_id=5875465628285931233>✈️</emoji> <b>Telegram ping:</> <code>{ping}</>ms\n<emoji document_id=5900104897885376843>🕓</emoji> <b>Uptime: {uptime}</>",
    ...
    async def ping(self, message: Message):
        ...
        await utils.answer(message, f'{self.config["ping_text"].format(ping=round((time.perf_counter_ns() - start) / 10**6, 3), uptime=utils.formatted_uptime(), ping_hint="", hostname=subprocess.run(["hostname"], stdout=subprocess.PIPE).stdout.decode().strip(), user=subprocess.run(["whoami"], stdout=subprocess.PIPE).stdout.decode().strip())}\n\n{self.config["hint"] if random.choice([0, 0, 1]) == 1 else ""}')
    ...
    async def client_ready(self):
        chat, _ = await utils.asset_channel(
            ...
            silent=True,
            invite_bot=True,
            avatar="https://raw.githubusercontent.com/Neotele-Studio/Hikka/refs/heads/master/assets/logs.jpg",
        )
